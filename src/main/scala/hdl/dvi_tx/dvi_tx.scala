...
class DVI_TX_Top() extends BlackBox {
    val io = IO(new Bundle {
        ...
        val I_rgb_r = Input(Vec(8, Bool()))
        val I_rgb_g = Input(Vec(8, Bool()))
        val I_rgb_b = Input(Vec(8, Bool()))
        val O_tmds_clk_p = Output(Bool())
        val O_tmds_clk_n = Output(Bool())
        val O_tmds_data_p = Output(Vec(3, Bool()))
        val O_tmds_data_n = Output(Vec(3, Bool()))
    })
  /*val VCC = Wire(Bool()) 
  val GND = Wire(Bool()) 
