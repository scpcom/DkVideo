...
class Video_Frame_Buffer_Top() extends BlackBox {
    val io = IO(new Bundle {
        ...
        val I_vin0_data = Input(Vec(16, Bool()))
        ...
        val O_vout0_data = Output(Vec(16, Bool()))
        val O_vout0_fifo_empty = Output(Bool())
        val O_cmd = Output(Bool())
        val O_cmd_en = Output(Bool())
        val O_addr = Output(Vec(22, Bool()))
        val O_wr_data = Output(Vec(32, Bool()))
        val O_data_mask = Output(Vec(4, Bool()))
        val I_rd_data_valid = Input(Bool())
        val I_rd_data = Input(Vec(32, Bool()))
